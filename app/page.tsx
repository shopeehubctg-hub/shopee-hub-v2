  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {section==="advertising" && <div className="page"><div className="page-title"><div><p className="kicker">ADVERTISING</p><h2>Campaign performance</h2></div><span className="period">This month</span></div>
        <section className={`ad-funds-card ${adFunds.balanceStatus}`}><div className="ad-funds-status"><span>{adFunds.syncStatus === "delayed" ? "Data delayed" : (adFunds.lowBalance ? `Top-up ${formatRinggit(adFunds.recommendedTopUp)} required` : "Ads healthy")}</span><small>{adFunds.topUpOwner === "shopee_hub" ? "Managed by Shopee Hub" : (adFunds.approvalRequired ? "Approval needed" : "Client action")}</small></div><div><span>Ad Balance</span><strong>{formatRinggit(adFunds.balance, 2)}</strong></div><div><span>Daily Spend</span><strong>{formatRinggit(adFunds.averageDailySpend30d, 2)}</strong></div><div><span>Runway</span><strong>{adFunds.runwayDays == null ? "—" : `${Math.floor(adFunds.runwayDays)} days`}</strong></div><div><span>Top-up</span><strong>{formatRinggit(adFunds.recommendedTopUp)}</strong></div><time>{adFunds.sourceUpdatedAt ? `Last updated ${adFunds.sourceUpdatedAt}` : "Last update unavailable"}</time></section>
