...
const portalUrl = "https://brandipp.shopee.com/case-management/management/list";
...
export function FakeSellerReport({storeName,allStores,cases}:{storeName:string;allStores:boolean;cases?:FakeSellerCase[]}){
  ...
  const scoped=useMemo(()=>source.filter(item=>allStores||item.store===storeName),[source,allStores,storeName]);
  ...
  return <div className="fake-seller-module">
    <section className="fake-seller-hero"><div><p className="kicker">BRAND PROTECTION</p><h2>Fake Seller Reporting</h2><p>Weekly proof of the sellers and listings reported on behalf of {allStores?"all projects":storeName}.</p></div><a href={portalUrl} target="_blank" rel="noopener noreferrer">Open Shopee Case Management ↗</a></section>
    ...
    <section className="protection-breakdown card"><div><p className="kicker">CASE OUTCOME</p><h3>Enforcement result</h3></div>{["Approved","Partially Approved","Rejected","Under Review"].map(label=>{const count=filtered.filter(x=>x.status===label).length;return <div className="outcome" key={label}><span className={`case-status ${statusTone[label as FakeSellerCase["status"]]}`}>{label}</span><b>{count}</b><i><span style={{width:`${casesSubmitted?count/casesSubmitted*100:0}%`}}/></i></div>})}</section>
