...
function detectCategory(meta:Meta){
  const ratio=meta.width/meta.height;
  if(meta.width===1200&&meta.height===518)return "Cover Photo";
  if(meta.width===1000&&meta.height===2000)return "Product Description 图";
  if(meta.width<=1200&&meta.height>meta.width)return "Shop Banner / Description 图";
  if(Math.abs(ratio-1)<.04)return "9 张产品图 / Promo 图";
  return "Shop Banner / Cover Photo";
}
function technical(meta:Meta,category:string):Finding[]{
  const max=/Cover Photo|Logo/.test(category)?1048576:2097152;
  ...
  const valid=category==="Cover Photo"?meta.width===1200&&meta.height===518:category==="Product Description 图"?meta.width===1000&&meta.height===2000:/9 张|Promo/.test(category)?meta.width===1080&&meta.height===1080:meta.width<=1200&&meta.height<=2200;
  return [size,valid?{level:"pass",title:"画布尺寸",detail:meta.width+" × "+meta.height+"px，符合识别类别标准。"}:{level:"fail",title:"画布尺寸不符合",detail:"目前为 "+meta.width+" × "+meta.height+"px，请按 "+category+" 标准调整。"},{level:"warning",title:"Logo／Watermark 对照",detail:"尚未配置这个品牌的标准参考文件，需由 Shopee Hub 确认。"}];
}
async function analyse(files:File[],metas:Meta[],categories:string[]){
  const key=process.env.OPENAI_API_KEY;if(!key)return null;
  const content:Array<Record<string,unknown>>=[{type:"input_text",text:"Review these Shopee images. Return JSON: {images:[{productType,detectedCategory,grammar:[{level,title,detail}],creative:[{level,title,detail}]}],advice:[string]}. Same image order. Grammar and creative levels may only be pass or warning. OCR English spelling and grammar, prohibited URL/email/phone/address/other platform/Return & Refund/non-English text and selling price; assess product type, product prominence, mobile readability, 2-3 key messages, USP, pain point, end result, ingredients, scene feeling and consistency. Give 3-6 concise actionable Chinese advice. Metadata: "+JSON.stringify(metas.map((m,i)=>({...m,category:categories[i]})))}];
  ...
  const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:"Bearer "+key,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-5-mini",input:[{role:"user",content}],text:{format:{type:"json_object"}}})});
...
export async function POST(request:Request){
  ...
  const categories=metas.map(detectCategory);const ai=await analyse(files,metas,categories) as null|{images?:Array<{productType?:string;detectedCategory?:string;grammar?:Finding[];creative?:Finding[]}>;advice?:string[]};
  const reviewId=crypto.randomUUID(),createdAt=new Date().toISOString();
  const images=files.map((file,index)=>{const t=technical(metas[index],categories[index]),a=ai?.images?.[index];return{id:crypto.randomUUID(),fileName:file.name,detectedCategory:a?.detectedCategory||categories[index],productType:a?.productType||"自动识别待 Shopee Hub 确认",grammar:a?.grammar?.length?a.grammar:[{level:"warning" as const,title:"AI 文字检查待连接",detail:"技术检查已完成；AI 文字服务连接后会自动显示完整结果。"}],technical:t,creative:a?.creative?.length?a.creative:[{level:"warning" as const,title:"等待 Shopee Hub 检查",detail:"画面吸引力、场景感和卖点清晰度需要最终人工确认。"}]}})
  ...
  await db.insert(designReviews).values({id:reviewId,tenantId:member.tenantId,storeId:String(form.get("storeId")||"all"),submittedBy:user.email,status,summary:{advice,imageCount:images.length},createdAt});
