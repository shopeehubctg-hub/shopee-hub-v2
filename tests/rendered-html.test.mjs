...
test("price calculator is available in navigation with all required outputs", async () => {
  ...
  assert.match(calculator, /跟 Meta 实际到手价一样/);
  assert.match(calculator, /顾客价与 Meta 一样/);
  assert.match(calculator, /顾客价比 Meta 便宜/);
